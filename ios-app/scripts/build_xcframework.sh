...
combine_ios_libs() {
    ...
    # The common library was renamed to llama-common upstream (produces
    # libllama-common.a). Hard-coding the stale libcommon.a made libtool
    # silently drop it (it runs with 2>/dev/null), causing undefined-symbol
    # link errors for common_log_*. Keep a fallback to libcommon.a for older
    # submodule pins.
    local common_lib="${build_dir}/common/${release_dir}/libllama-common.a"
    if [[ ! -f "$common_lib" ]]; then
        common_lib="${build_dir}/common/${release_dir}/libcommon.a"
    fi
    local libs=(
    ...
    # Upstream common/CMakeLists.txt omits the generated build-info.cpp from
    # the llama-common target, so llama_commit()/llama_build_info()/etc. are
    # left undefined at link time. Generate + compile it from the upstream
    # template (read-only — we never modify submodule files) and merge it in.
    local bi_commit bi_number
    bi_commit="$(cd "${SUBMODULE_DIR}" && git rev-parse --short HEAD 2>/dev/null || echo unknown)"
    bi_number="$(cd "${SUBMODULE_DIR}" && git rev-list --count HEAD 2>/dev/null || echo 1)"
    sed -e "s/@LLAMA_BUILD_NUMBER@/${bi_number}/g" \
        -e "s/@LLAMA_BUILD_COMMIT@/${bi_commit}/g" \
        -e "s/@BUILD_COMPILER@/AppleClang/g" \
        -e "s/@BUILD_TARGET@/apple-ios/g" \
        "${SUBMODULE_DIR}/common/build-info.cpp.in" > "${temp_dir}/build-info.cpp"
    xcrun -sdk "$sdk" clang++ -c "${temp_dir}/build-info.cpp" \
        -I"${SUBMODULE_DIR}/common" \
        $arch_flags $min_version_flag \
        -o "${temp_dir}/build-info.o"
    xcrun libtool -static -o "${temp_dir}/combined.a" "${temp_dir}/combined.a" "${temp_dir}/build-info.o" 2>/dev/null
...
# Ensure the destination directory exists (it is not tracked in git, so a
# fresh checkout / submodule bump leaves it absent and `cp` would fail).
mkdir -p "${DEST_DIR}"
